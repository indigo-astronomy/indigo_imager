...
# Ensure a persistent self-signed code signing identity exists in the login keychain.
ensure_local_signing_identity() {
  if security find-identity -v -p codesigning | grep -Fq "$SIGNING_IDENTITY"; then
    return 0
  fi

  echo "Creating self-signed code signing identity: $SIGNING_IDENTITY"
  TMPDIR=$(mktemp -d)
  CERT_SUBJ="/CN=$SIGNING_IDENTITY"

  openssl req -x509 -newkey rsa:2048 -days 3650 -nodes -subj "$CERT_SUBJ" -keyout "$TMPDIR/key.pem" -out "$TMPDIR/cert.pem" >/dev/null 2>&1 || {
    echo "OpenSSL failed to create cert"; rm -rf "$TMPDIR"; return 1
  }

  openssl pkcs12 -export -inkey "$TMPDIR/key.pem" -in "$TMPDIR/cert.pem" -name "$SIGNING_IDENTITY" -passout pass:localpass -out "$TMPDIR/cert.p12" >/dev/null 2>&1 || {
    echo "OpenSSL failed to export PKCS#12"; rm -rf "$TMPDIR"; return 1
  }

  # Import into login keychain and allow codesign access (-A)
  security import "$TMPDIR/cert.p12" -k "$HOME/Library/Keychains/login.keychain-db" -P localpass -A >/dev/null 2>&1 || {
    echo "Failed to import cert into keychain"; rm -rf "$TMPDIR"; return 1
  }

  # remove temp files
  rm -rf "$TMPDIR"
  # give the system a moment to recognize the identity
  sleep 1
  if security find-identity -v -p codesigning | grep -Fq "$SIGNING_IDENTITY"; then
    echo "Created and imported $SIGNING_IDENTITY"
    return 0
  else
    echo "Imported cert not found in identity list"
    return 1
  fi
}
...
if ! ensure_local_signing_identity; then
  echo "Failed to create a local signing identity. Create one manually in Keychain Access named '$SIGNING_IDENTITY' and re-run the script."
  exit 1
fi

# we have a persistent identity now
SIGNING_MODE="identity"
