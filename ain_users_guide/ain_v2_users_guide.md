# Ain INDIGO Imager version 2 - Users Guide
...
##### Enable Dithering
Enables and disables ditherng.

##### Dithering strategy
The dropdown menu lists the available dihering strategies to choose from.
...
Sequences are implemented in Ecma Script using the *INDIGO Scripting Agent* and are executed on the selected *Scripting Agent*. The user can add predefined actions to be executed. The sequence shown below will setup the slected gear, enable merifian flip at meridian (no offset), set the object name ("Rosette"), calibrate and start guiding, and then take R, G and B exposures in batches of 2 in a loop that will repeat 100 times. Then it will stop guiding park the mount etc.

![](images_v2/sequence_main.png)

### Editing sequence
Each sequence consists of the actions, which will be executed sequentually. Each action is described by zero or several proprieties like: Filter, exposure time, frame type etc.
...
In order to remove actions press the **(x)** buttom on the right side of the action widget.
...
The sequence being edited or loaded will be uploaded to the *Scripting Agebt* only when the sequence is started. Before that all the changes will remain in the client only. Because of that, if the user changes the sequence while it is running the changes will not take effect.
...
**Abort** button will stop the current sequence. And the completed actions will be decorated with green while the aborted one will be decorated red. 
...
Progress can be monitored using the three progress bars and the operation indicator as shown above. The first progress bar shows the elapsed time of each exposure. The second one shows the completed exposures in the current batch. The third one shows the sequece completion in percents. Please note that the percents represent the completed actions and are not releated to time.
...
**[&Sigma;]** buttom will update the total exposure time before the sequence is executed.
...
In the **Rotator**  drop-down menu all available rotators will be listed.
The selected rotator will be used.

The **Position** can be enteredin the spinbox or by rotating the knob. The **[>]** button will move the rotator to the selected angle. The **[0]** button will sync the rotator to the selected angle. 
...
**Start Derotation** will start the compensation for the filed rotation introduced by the Alt-Azimutal mounts.
